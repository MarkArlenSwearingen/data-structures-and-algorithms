...
const oddValues = (arr) => {
  // Solution code here...
  let odds = arr.filter( value => {return (value % 2)});
  return odds;
};
...
const filterStringsWithVowels = (arr) => {
  ...
  let regex = /[aeiou]/gim;
  let result = [];
  let words = arr.filter(function (value) {
...
const notInFirstArray = (forbiddenValues, arr) => {
  // Solution code here...
  return arr.filter(function(badvalue) {
    return !forbiddenValues.includes(badvalue);
  })
};
...
const evenOddNumericValues = (arr) => {
  ...
  return numbers.map(value => (value % 2 === 0)? 'even' : 'odd')
};
